...
const MyVerticallyCenteredModal = (props) => {
    ...
    const {checkTags, setCheckTags, action, setAction, check, setCheck, modalShowObj, tasks, setTasks,
        setIsTitleChange, isTitleChange,isDescription,setIsDescription,} = props; // деструктуризация - для вывода ключей в отдельные переменные
        ...
        >
            ...
            <Modal.Body>
                ...
                <div className="popup__description-block" >
                    {
                        isDescription? <textarea className='popup__description-textarea' onChange={(e)=>setDescription(e.target.value)} defaultValue={modalShowObj.description}/>
                        : <div style={{display:"flex",flexDirection:'column',justifyContent:'space-between',paddingBottom:'20px'}}>

                                <p>
                                    {modalShowObj.description}
                                </p>
                            <span className='popup__description'  onClick={()=>setIsDescription(true)}>
                                {
                            ...
                            <>
                                ...
                                {tags.map((item) => (
                                    <div style={{
                                        ...
                                        <div>
                                            <AiFillTag style={{ color: item === 'Home' ? 'yellow' : item === 'Work' ? 'blue' :
                                                    item === 'Personal' ? 'green' : 'gray'}}/>
                                            <span style={{marginLeft: '10px', fontSize: '18px', }}>{item}</span>
                                        </div>
                                        <div style={{display: 'flex', alignItems: 'center' }}>
                                            <input checked={checkTags.includes(item)} onChange={() => checkTagsHandler(item)}  type="checkbox" style={{width: '20px', height: '20px'}}/>
                                            <span style={{marginLeft: '10px', cursor: 'pointer' ,fontSize: '20px'} } onClick={() => delTag(item)}>X</span>
                                        </div>
                                    </div>
                                ))}
                            </>
